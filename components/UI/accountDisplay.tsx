...
export const AccountDisplay = ({image, username, name, setLoginModal}:PropTypes) => {
    ...
    return (
        <div className='flex items-center gap-2 rounded-full h-16 w-60 bg-slate-900 p-2'>
            <Image width={1080} height={1080} className='rounded-full w-10 h-10 aspect-square' src={image} alt='image' />
            <div className='w-[65%] flex flex-col gap-2'>
                <h3 className='font-bold text-lg max-w-36 overflow-hidden'>{name}</h3>
                <h3 className='leading-tight text-slate-500 text-sm max-w-36 overflow-hidden'>@{username}</h3>
            </div>
            <button onClick={()=>{signOut({ callbackUrl: '/' })}} className='w-[10%] flex justify-center text-xl duration-200 hover:scale-110 items-center'>
                <IoIosLogOut />
            </button>
        </div>
    )
