...
export const TextInput = ({content, setContent, heading, placeholder, type, required, limit}:{type?: string, required:boolean, content:string, setContent:Dispatch<string>, heading:string, placeholder:string, limit?:number}) => {
  return (
    <div className="w-full text-start flex flex-col">
          ...
          <h2 className={`text-sm text-semibold text-nifty-gray-1 order-first mt-4 peer-focus:font-semibold duration-200 peer-focus:text-slate-400 text-slate-500/80 `}>{heading} {required && <span className="text-red-500 font-semibold ml-1">*</span>} {limit && content.length+"/"+limit}</h2>
      </div>
  )
